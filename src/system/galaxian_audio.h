...
//   - VCO de fond modulé par DAC 4-bit (ports 0x6004-0x6007)
// ============================================================================
...
public:
    ...
    void reset() {
        ...
        // VCO fond sonore (DAC 4-bit → 60-240 Hz)
        m_bg_freq = 120.0f;
        m_dac_value = 0x08;
    }

    // Contrôle hardware (écritures Z80) — ports 0x6800-0x6807
...
private:
    float process_sample() {
        ...
        // 1. Oscillateur de fond VCO (DAC modulé) — toujours présent
        output += process_bg_vco();

        // 2. Oscillateurs tones (ondes carrées)
        ...
        // 3. Tir FIRE (VCO modulé par enveloppe RC)
        ...
        // 4. Bruit HIT/Explosion (LFSR + enveloppe RC)
    ...
    // Oscillateur de fond VCO piloté par le DAC (60-240 Hz)
    float process_bg_vco() {
        if (m_bg_freq <= 0.0f) return 0.0f;
        m_phase[3] += m_bg_freq / SAMPLE_RATE;
        if (m_phase[3] >= 1.0f) m_phase[3] -= 1.0f;
        return (m_phase[3] < 0.5f) ? 0.3f : -0.3f; // Amplitude réduite pour le fond
    }
    ...
    // Fréquence VCO fond sonore (modulée par DAC, 60-240 Hz)
    float m_bg_freq = 120.0f;
    uint8_t m_dac_value = 0x08; // Valeur moyenne au reset
